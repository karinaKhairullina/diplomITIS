...
from .utils.visualization import plot_anomaly_score_histogram, plot_umap_with_anomaly_scores
        ...
        try:
            ...
            if file.name.endswith('.csv'):
                ...
                try:
                    ...
                    return JsonResponse({"error": "Ошибка чтения файла. Проверьте корректность структуры файла."}, status=400)
            else:
            ...
            for chunk in chunks:
                ...
                # Новая логика: проверка, есть ли хотя бы один признак
                recognized_columns = set(chunk.columns) & set(REQUIRED_FEATURES)
                if not recognized_columns:
                    return JsonResponse({
                        "error": "Отсутствуют наименования признаков"
                    }, status=400)
